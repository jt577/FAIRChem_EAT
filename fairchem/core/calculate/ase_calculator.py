...
class FAIRChemCalculator(Calculator):
    ...
    def calculate(
        ...
        if len(atoms) == 1 and sum(atoms.pbc) == 0:
            ...
            # Do not hold onto device tensors after an EAT call. The next prediction
            # will repopulate this handle before the calculator asks for eat_grad.
            if hasattr(self.predictor, "_last_data_device"):
                self.predictor._last_data_device = None
