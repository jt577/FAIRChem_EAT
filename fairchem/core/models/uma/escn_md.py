...
class MLP_EFS_Head(nn.Module, HeadInterface):
    ...
    def forward(
        ...
        if isinstance(flag, (list, tuple)):                                             
            ...
            flag = bool(flag)  
        # Low-memory EAT relaxation: forces are needed as numerical first
        # derivatives for ionic relaxation, but ordinary EAT does not need the
        # forces themselves to remain differentiable with respect to species
        # weights. Keep the energy graph only until the calculator takes
        # dE/d(eat_weights).
        if flag and (self.regress_stress or not self.regress_forces):    
            forces = (                                                                                                                    
                -1                                                                                                                      
                * torch.autograd.grad(                                                                                                  
                    energy_part.sum(), data["pos"], create_graph=False, retain_graph=True,  
                )[0]                                                                                                                    
            )         
            if gp_utils.initialized():
        ...
        if self.regress_stress:
            ...
            if flag:    
                forces = (                                                                                                                    
                    -1                                                                                                                      
                    * torch.autograd.grad(                                                                                                  
                        energy_part.sum(), data["pos"], create_graph=False, retain_graph=True,   # retain graph for EAT grad calc   
                    )[0]                                                                                                                    
                )                                                                                                                           
            # -------------------- END EAT MODIFICATION --------------------
            else:
