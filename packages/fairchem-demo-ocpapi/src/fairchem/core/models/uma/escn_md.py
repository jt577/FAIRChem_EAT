...
class eSCNMDBackbone(nn.Module, MOLEInterface):
    ...
    def _generate_graph(self, data_dict):
        ...
        if gp_utils.initialized():
            ...
            if "eat_weights_full" in data_dict:                                                 # -------------------- EAT MODIFICATION --------------------
                data_dict["eat_weights"] = data_dict["eat_weights_full"][                       # -------------------- EAT MODIFICATION --------------------
                    graph_dict["node_partition"]                                                # -------------------- EAT MODIFICATION --------------------
                ]                                                                               # -------------------- EAT MODIFICATION --------------------
    ...
    def forward(self, data_dict: AtomicData) -> dict[str, torch.Tensor]:
        ...
        if "eat_weights" in data_dict:                                               # -------------------- EAT MODIFICATION --------------------
            data_dict["eat_weights_full"] = data_dict["eat_weights"]                 # -------------------- EAT MODIFICATION --------------------
        ...
        with record_function("edge embedding"):
            ...
            # source_embedding = self.source_embedding(                                 # -------------------- ORIGINAL CODE --------------------
            #     data_dict["atomic_numbers_full"][graph_dict["edge_index"][0]]         # -------------------- ORIGINAL CODE --------------------
            # )                                                                         # -------------------- ORIGINAL CODE --------------------
            # target_embedding = self.target_embedding(                                 # -------------------- ORIGINAL CODE --------------------
            #     data_dict["atomic_numbers_full"][graph_dict["edge_index"][1]]         # -------------------- ORIGINAL CODE --------------------
            # )                                                                         # -------------------- ORIGINAL CODE --------------------
            source_embedding, target_embedding = self._get_edge_endpoint_embeddings(    # -------------------- EAT MODIFICATION --------------------
                data_dict, graph_dict                                                   # -------------------- EAT MODIFICATION --------------------
            )                                                                           # -------------------- EAT MODIFICATION --------------------
            x_edge = torch.cat(
    ...
    def _get_node_sphere_embedding(self, data_dict):                                        # -------------------- EAT MODIFICATION --------------------
        """                                                                                 # -------------------- EAT MODIFICATION --------------------
        Returns per-node scalar embedding of shape (num_nodes, sphere_channels).            # -------------------- EAT MODIFICATION --------------------
        If `eat_weights` is present, uses it to make a linear combination of                # -------------------- EAT MODIFICATION --------------------
        element embeddings. Otherwise falls back to standard atomic_numbers lookup.         # -------------------- EAT MODIFICATION --------------------
        """                                                                                 # -------------------- EAT MODIFICATION --------------------
        if "eat_weights" in data_dict:                                                      # -------------------- EAT MODIFICATION --------------------
            weights = data_dict["eat_weights"]  # (num_nodes, n_elems)                      # -------------------- EAT MODIFICATION --------------------
            elem_emb = self.sphere_embedding.weight  # (max_num_elements, sphere_channels)  # -------------------- EAT MODIFICATION --------------------
            # sanity checks                                                                 # -------------------- EAT MODIFICATION --------------------
            assert weights.dim() == 2                                                       # -------------------- EAT MODIFICATION --------------------
            assert weights.shape[1] == elem_emb.shape[0], (                                 # -------------------- EAT MODIFICATION --------------------
                f"eat_weights second dim {weights.shape[1]} "                               # -------------------- EAT MODIFICATION --------------------
                f"!= sphere_embedding size {elem_emb.shape[0]}"                             # -------------------- EAT MODIFICATION --------------------
            )                                                                               # -------------------- EAT MODIFICATION --------------------
            # match dtype just in case                                                      # -------------------- EAT MODIFICATION --------------------
            weights = weights.to(elem_emb.dtype)                                            # -------------------- EAT MODIFICATION --------------------
            # (num_nodes, sphere_channels)                                                  # -------------------- EAT MODIFICATION --------------------
            return weights @ elem_emb                                                       # -------------------- EAT MODIFICATION --------------------
        else:                                                                               # -------------------- EAT MODIFICATION --------------------
            return self.sphere_embedding(data_dict["atomic_numbers"])                       # -------------------- EAT MODIFICATION --------------------

    def _get_edge_endpoint_embeddings(self, data_dict, graph_dict):                         # -------------------- EAT MODIFICATION --------------------                       
        """                                                                                 # -------------------- EAT MODIFICATION --------------------
        Returns source_embedding, target_embedding for edges.                               # -------------------- EAT MODIFICATION --------------------
        Uses EAT weights if present, else falls back to integer embeddings.                 # -------------------- EAT MODIFICATION --------------------
        """                                                                                 # -------------------- EAT MODIFICATION --------------------
        if "eat_weights_full" in data_dict:                                                 # -------------------- EAT MODIFICATION --------------------
            weights_full = data_dict["eat_weights_full"]  # (N, Z)                          # -------------------- EAT MODIFICATION --------------------
            src_w = weights_full[graph_dict["edge_index"][0]]  # (E, Z)                     # -------------------- EAT MODIFICATION --------------------
            tgt_w = weights_full[graph_dict["edge_index"][1]]  # (E, Z)                     # -------------------- EAT MODIFICATION --------------------
            assert weights_full.shape[1] == self.source_embedding.weight.shape[0]           # -------------------- EAT MODIFICATION --------------------
            src_emb = src_w @ self.source_embedding.weight   # (E, edge_channels)           # -------------------- EAT MODIFICATION --------------------
            tgt_emb = tgt_w @ self.target_embedding.weight   # (E, edge_channels)           # -------------------- EAT MODIFICATION --------------------
            return src_emb, tgt_emb                                                         # -------------------- EAT MODIFICATION --------------------
        else:                                                                               # -------------------- EAT MODIFICATION --------------------
            src_emb = self.source_embedding(                                                # -------------------- EAT MODIFICATION --------------------
                data_dict["atomic_numbers_full"][graph_dict["edge_index"][0]]               # -------------------- EAT MODIFICATION --------------------
            )                                                                               # -------------------- EAT MODIFICATION --------------------
            tgt_emb = self.target_embedding(                                                # -------------------- EAT MODIFICATION --------------------
                data_dict["atomic_numbers_full"][graph_dict["edge_index"][1]]               # -------------------- EAT MODIFICATION --------------------
            )                                                                               # -------------------- EAT MODIFICATION --------------------
            return src_emb, tgt_emb                                                         # -------------------- EAT MODIFICATION --------------------
...
class MLP_EFS_Head(nn.Module, HeadInterface):
    ...
    def forward(
        ...
        # Normalize enable_eat_grad to a plain bool                                     # -------------------- EAT MODIFICATION --------------------
        flag = getattr(data, "enable_eat_grad", False)                                  # -------------------- EAT MODIFICATION --------------------
        if isinstance(flag, (list, tuple)):                                             # -------------------- EAT MODIFICATION --------------------
            flag = any(flag)                                                            # -------------------- EAT MODIFICATION --------------------
        elif torch.is_tensor(flag):                                                     # -------------------- EAT MODIFICATION --------------------
            assert flag.numel() == 1, "enable_eat_grad tensor must be scalar"           # -------------------- EAT MODIFICATION --------------------
            flag = bool(flag.item())                                                    # -------------------- EAT MODIFICATION --------------------
        else:                                                                           # -------------------- EAT MODIFICATION --------------------
            flag = bool(flag)                                                           # -------------------- EAT MODIFICATION --------------------

        if self.regress_stress:
            if flag:                                                                    # -------------------- EAT MODIFICATION --------------------
                grads = torch.autograd.grad(                                            # -------------------- EAT MODIFICATION --------------------
                    [energy_part.sum()],                                                # -------------------- EAT MODIFICATION --------------------
                    [data["pos_original"], emb["displacement"]],                        # -------------------- EAT MODIFICATION --------------------
                    create_graph=self.training,                                         # -------------------- EAT MODIFICATION --------------------
                    retain_graph=True,          # retain graph for EAT grad calc        # -------------------- EAT MODIFICATION --------------------
                )                                                                       # -------------------- EAT MODIFICATION --------------------
            else:
            ...
            if flag:                                                                                                                        # -------------------- EAT MODIFICATION --------------------
                forces = (                                                                                                                  # -------------------- EAT MODIFICATION --------------------   
                    -1                                                                                                                      # -------------------- EAT MODIFICATION --------------------
                    * torch.autograd.grad(                                                                                                  # -------------------- EAT MODIFICATION --------------------
                        energy_part.sum(), data["pos"], create_graph=self.training, retain_graph=True,   # retain graph for EAT grad calc   # -------------------- EAT MODIFICATION --------------------
                    )[0]                                                                                                                    # -------------------- EAT MODIFICATION --------------------
                )                                                                                                                           # -------------------- EAT MODIFICATION --------------------
            else:
                forces = (
