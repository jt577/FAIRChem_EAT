    ...
    def set_MOLE_coefficients(
        ...
        with torch.autocast(device_type=atomic_numbers_full.device.type, enabled=False):
            ...
            if self.use_composition_embedding:
                if eat_weights_full is not None:                                                # -------------------- EAT MODIFICATION --------------------
                    # EAT path: composition from soft weights                                   # -------------------- EAT MODIFICATION --------------------
                    # eat_weights_full: (N, max_num_elements)                                   # -------------------- EAT MODIFICATION --------------------
                    # composition_embedding.weight: (max_num_elements, sphere_channels)         # -------------------- EAT MODIFICATION --------------------
                    W = self.composition_embedding.weight  # (Z, C)                             # -------------------- EAT MODIFICATION --------------------
                    composition_by_atom = eat_weights_full.to(W.dtype) @ W  # (N, C)            # -------------------- EAT MODIFICATION --------------------
                else:
