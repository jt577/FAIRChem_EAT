...
class ElementReferences(nn.Module):
    ...
    def compute_references(batch, tensor, elem_refs, operation):
        ...
        with torch.autocast(elem_refs.device.type, enabled=False):
            if hasattr(batch, "eat_weights_full"):                                      # -------------------- EAT MODIFICATION --------------------
                # EAT: elem_refs is per-element; compute per-atom refs                  # -------------------- EAT MODIFICATION --------------------
                # batch.eat_weights_full: (N, Z)                                        # -------------------- EAT MODIFICATION --------------------
                weights = batch.eat_weights_full.to(elem_refs.device)                   # -------------------- EAT MODIFICATION --------------------
                # elem_refs: shape compatible with Z, e.g. (Z,) or (Z, D, ...)          # -------------------- EAT MODIFICATION --------------------
                # simplest 1D case:                                                     # -------------------- EAT MODIFICATION --------------------
                per_atom_refs = (weights * elem_refs.unsqueeze(0)).sum(dim=1)           # -------------------- EAT MODIFICATION --------------------
                refs = torch.zeros(                                                     # -------------------- EAT MODIFICATION --------------------
                    tensor.shape, dtype=elem_refs.dtype, device=tensor.device           # -------------------- EAT MODIFICATION --------------------
                ).scatter_reduce(                                                       # -------------------- EAT MODIFICATION --------------------
                    0,                                                                  # -------------------- EAT MODIFICATION --------------------
                    batch.batch_full,                                                   # -------------------- EAT MODIFICATION --------------------
                    per_atom_refs,                                                      # -------------------- EAT MODIFICATION --------------------
                    reduce="sum",                                                       # -------------------- EAT MODIFICATION --------------------
                )                                                                       # -------------------- EAT MODIFICATION --------------------
            else:
                refs = torch.zeros(
