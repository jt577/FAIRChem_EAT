...
class FAIRChemCalculator(Calculator):
    ...
    def calculate(
        ...
        if len(atoms) == 1 and sum(atoms.pbc) == 0:
            ...
            # Normalize EAT grad flag for this system                                           # -------------------- EAT MODIFICATION --------------------
            enable_eat_grad = getattr(data_object, "enable_eat_grad", False)                    # -------------------- EAT MODIFICATION --------------------
            if isinstance(enable_eat_grad, (list, tuple)):                                      # -------------------- EAT MODIFICATION --------------------
                enable_eat_grad = any(enable_eat_grad)                                          # -------------------- EAT MODIFICATION --------------------
            elif torch.is_tensor(enable_eat_grad):                                              # -------------------- EAT MODIFICATION --------------------
                assert enable_eat_grad.numel() == 1, "enable_eat_grad tensor must be scalar"    # -------------------- EAT MODIFICATION --------------------
                enable_eat_grad = bool(enable_eat_grad.item())                                  # -------------------- EAT MODIFICATION --------------------
            else:                                                                               # -------------------- EAT MODIFICATION --------------------
                enable_eat_grad = bool(enable_eat_grad)                                         # -------------------- EAT MODIFICATION --------------------
            ...
            for calc_key in self.implemented_properties:
                if calc_key == "energy":
                    # Extract EAT gradient if available and enabled                             # -------------------- EAT MODIFICATION --------------------
                    E_tensor = pred[calc_key]                                                   # -------------------- EAT MODIFICATION --------------------
                    if "eat_weights" in batch and enable_eat_grad:                              # -------------------- EAT MODIFICATION --------------------           
                        eat_grad = torch.autograd.grad(                                         # -------------------- EAT MODIFICATION --------------------
                            E_tensor.sum(),       # scalar                                      # -------------------- EAT MODIFICATION --------------------
                            batch.eat_weights,    # tensor we set requires_grad=True on         # -------------------- EAT MODIFICATION --------------------
                            create_graph=False                                                  # -------------------- EAT MODIFICATION --------------------
                        )[0]                                                                    # -------------------- EAT MODIFICATION --------------------
                        self.results["eat_grad"] = eat_grad.detach().cpu().numpy()              # -------------------- EAT MODIFICATION --------------------
